...
// import CheckBox from '@react-native-community/checkbox' //wrap nalang sa touchableOpac
...
const taskSchema = z.object({
  newTask: z.string().min(5, { message: "Task Name is Required" }),
});

const editTaskSchema = z.object({
  editingTask: z.string().min(5, { message: "Task Name is Required" }),
});
...
export default function ToDoList() {
  ...
  const [clearAllModalVisible, setClearAllModalVisible] = useAtom(
    clearAllModalVisibleAtom
  );
  ...
  const addTask = (data) => {
    setTasks([
      ...tasks,
      { id: Date.now().toString(), text: data.newTask, checked: false },
    ]);
    reset({ newTask: "" });
  };
  ...
  const editTask = (data) => {
    setTasks(
      tasks.map((task) =>
        task.id === selectedTaskId ? { ...task, text: data.editingTask } : task
      )
    );
  ...
  return (
    <View style={styles.container}>
      ...
      />
      {errors.newTask && (
        <Text style={styles.errorText}>{errors.newTask.message}</Text>
      )}
      <TouchableOpacity style={styles.button} onPress={handleSubmit(addTask)}>
        <Text style={styles.buttonText}>
          <Icon3 name="addfile" size={20} color="white" /> Add Task
        </Text>
      </TouchableOpacity>
      <FlatList
        data={tasks}
      ...
      {anyTaskChecked && (
        <TouchableOpacity
          style={styles.button}
          onPress={() => setTasks(tasks.filter((task) => !task.checked))}
        >
          <Text style={styles.buttonText}>
            {" "}
            <Icon2 name="delete" size={25} color="white" /> Delete Selected
          </Text>
        </TouchableOpacity>
      )}
      <TouchableOpacity
        style={[
          styles.button,
          { backgroundColor: tasks.length === 0 ? "#ccc" : "orange" },
        ]}
        onPress={() => setClearAllModalVisible(true)}
        disabled={tasks.length === 0}
      >
        <Text style={styles.buttonText}>
          <Icon2 name="delete-outline" size={25} color="white" /> Clear All
        </Text>
      </TouchableOpacity>
      ...
      <Modal visible={modalVisible} animationType="fade" transparent={true}>
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            {editErrors.editingTask && (
              <Text style={styles.errorText}>
                {editErrors.editingTask.message}
              </Text>
            )}
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity
                ...
                onPress={handleEditSubmit(editTask)}
              >
